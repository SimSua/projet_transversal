...
void send_on_rf(void)
{
	...
	/* Create a local copy */
	memcpy((char*)&(cc_tx_data[2]), "T", tx_len);
    memcpy((char*)&(cc_tx_data[3]), "H", tx_len);
    memcpy((char*)&(cc_tx_data[4]), "L", tx_len);
	...
	ret = cc1101_send_packet(cc_tx_data, (5));

#ifdef DEBUG
	uprintf(UART0, "Tx ret: %d\n\r", ret);
    uprintf(UART0, "RF: data lenght: %d.\n\r", cc_tx_data[0]);
    uprintf(UART0, "RF: destination: %x.\n\r", cc_tx_data[1]);
    uprintf(UART0, "RF: message: %c%c%c.\n\r", cc_tx_data[2],cc_tx_data[3],cc_tx_data[4]);
#endif
}
