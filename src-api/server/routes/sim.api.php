...
Route::apiResource('coordinates', 'SimCoordinateController')->middleware('throttle:1000,1');
Route::apiResource('fires','SimFireController')->middleware('throttle:1000,1');
Route::apiResource('fire-departments','SimFireDepartmentController')->middleware('throttle:1000,1');
Route::apiResource('vehicle-types','SimVehicleTypeController')->middleware('throttle:1000,1');
Route::apiResource('trucks','SimTruckController')->middleware('throttle:1000,1');
...
Route::get('/fire-departments/vehicles/{id}', function(SimFireDepartmentController $fireDepartmentController, int $id) {
    return $fireDepartmentController->getAllTrucks($id)->middleware('throttle:1000,1');
})->where(['id' => '[0-9]+'])->middleware('throttle:1000,1');
